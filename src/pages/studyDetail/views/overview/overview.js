...
import BiospecimenProfile from "./components/BiospecimenProfile/BiospecimenProfile.js";
...
const Overview = ({ classes, data }) => {
  ...
  const diagnoses = [
    ...(data?.studyDiagnosisByStudyShortName?.[0]?.ctep_disease_terms || []),
  ];
  const participantFileTypes = [
    ...(data?.StudyDataFileByStudyShortName?.[0]?.list_type || []),
  ];

  const { study_name, study_description, study_type, dates_of_conduct } = study;

  // Study data files / ZIP file
  const studyDataFiles =
    data?.StudyDataFileByStudyShortName?.[0]?.study_data_files || [];
  const zipFileData = studyDataFiles.find(
                ...
                <Grid container className={classes.detailContainerItems}>
                  ...
                  <Grid item xs={12} className={classes.detailContainerItem}>
                    <Grid item container direction="row">
                      ...
                      {associatedLinks
                        .sort((a, b) =>
                          customSorting(
                            a.associated_link_record_id,
                            b.associated_link_record_id
                          )
                        )
                        .map((link, index) => (
                          <Grid
                  ...
                  <Grid item xs={12} className={classes.detailContainerItem}>
                    <Grid item container direction="row">
                      ...
                      <Grid item xs={12} className={classes.content}>
                        Subject to the appropriate access controls, copies of
                        all Clinical Reports, Variant Reports, and Variant Call
                        Files associated with the Cancer Moonshot Biobank study
                        that are currently represented within the application
                        can be downloaded in the form of a .zip file by
                        selecting the ZIP FILE download option below.
                        {hasZipFile ? (
                          ...
                          <ZipDownloadView
                            ...
                            fileFormat={
                              zipFileData[
                                documentDownloadProps.fileFormatColumn
                              ]
                            }
                            fileName={
                              zipFileData[documentDownloadProps.fileNameColumn]
                            }
                            fileLocation={
                              zipFileData[
                                documentDownloadProps.fileLocationColumn
                              ]
                            }
                            toolTipTextFileDownload={
                              documentDownloadProps.toolTipTextFileDownload
                            }
                            iconFileDownload={
                              documentDownloadProps.iconFileDownload
                            }
                            iconUnauthenticated={
                              documentDownloadProps.iconUnauthenticated
                            }
                            toolTipTextUnauthenticated={
            ...
            <Grid item lg={7} md={8} sm={6} xs={12}>
              ...
              >
                ...
                >
                  ...
                  <Grid
                    container
                    className={classes.detailContainerCL}
                    tabIndex="0"
                  >
                    {diagnoses
                      .sort((a, b) => customSorting(a, b))
                      .map((diagnosis, index) => (
                        <Grid item xs={12} key={index}>
                          <span className={classes.content}>{diagnosis}</span>
                        </Grid>
                      ))}
                  </Grid>
                </Grid>
                    ...
                    <Grid item xs={12}>
                      <div className={classes.collection}>
                        {imageCollection
                          .sort((a, b) =>
                            customSorting(
                              a.image_collection_name,
                              b.image_collection_name
                            )
                          )
                          .map((image, index) => (
                            <div className={classes.collectionWrapper}>
                              ...
                              >
                                <ToolTip
                                  ...
                                  title={getAccessTypeString(
                                    image.collection_access
                                  )}
                                  placement="bottom"
                                >
                                  <span
                                    className={
                                      classes.collectionAndRepositorySpan
                                    }
                                  >
                      ...
                      <p className={classes.helpfulDirectionText}>
                        Scroll down to see more available
                      </p>
                    </Grid>
                  </Grid>
                </Grid>
